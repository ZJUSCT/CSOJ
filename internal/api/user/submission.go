...
func (h *Handler) submitToProblem(c *gin.Context) {
	...
	// Effective submission deadline: a matching tag override REPLACES the
	// default deadline (even if earlier); the latest matching override wins.
	// No match → the default *probSubmitEnd stands. Checked unconditionally so
	// an earlier override that has already passed still rejects the submission.
	effectiveEnd, _ := latestMatchingOverride(problem, user.Tags, *probSubmitEnd)
